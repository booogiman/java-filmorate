...
public class FilmController {
    ...
    @DeleteMapping
    public void delete(@Valid @RequestBody Film film) throws Exception {
    ...
    @GetMapping("/{id}")
    public Film getOne(@PathVariable Long id) throws Exception {
        return filmService.getFilmStorage().getFilmById(id);
    }

    // Метод (пользователь ставит лайк фильму)
    @PutMapping("/{id}/like/{userId}")
    public void addLike(@PathVariable Long id, @PathVariable Long userId) throws Exception {
        filmService.addLike(id, userId);
    }

    // Метод (пользователь удаляет лайк)
    @DeleteMapping("/{id}/like/{userId}")
    public void deleteLike(@PathVariable Long id, @PathVariable Long userId) throws Exception {
